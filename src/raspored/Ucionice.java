...
public class Ucionice {
    ...
    public void obrada() {
        ...
        for(int i=2; i<Raspored.podaci[0].length; i++) {
            for(int j=2; j<Raspored.podaci[0][i].length; j++) {
                if (Raspored.podaci[0][i][j] != null) {
                    String ucionica = Raspored.podaci[0][i][j];
    ...
    private void format(int red, int kolona) {
        String odeljenje = Raspored.podaci[0][kolona][red - 1];
        String predmet = Raspored.podaci[0][0][red - 1].replace('\n', ' ');
        int outi1, outi2=-1;
        for(int i=1; i<6; i++) 
